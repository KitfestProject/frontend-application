...
import { upstairsFrontRightSectionData } from "@/components/data/NairobiCinemaSeatData";
  ...
  return (
    <>
      <div className="flex flex-col gap-2 border-gray/50 rounded-lg p-5 relative items-center">
        {upstairsFrontRightSectionData.rows.map((row, rowIndex) => (
          <div key={rowIndex} className="flex gap-2">
            {row.seats.map((seat, seatIndex) => {
              const seatData = {
                ...
                price: seat.price,
                status: seat.status,
                position: upstairsFrontRightSectionData.location,
                description: upstairsFrontRightSectionData.description,
              };
              return (
                <Seat
                  key={seatIndex}
...
export default UpstairsFrontRowRightSection;
