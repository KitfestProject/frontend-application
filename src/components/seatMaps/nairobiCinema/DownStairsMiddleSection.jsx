...
import { downStairsMiddleSectionData } from "@/components/data/NairobiCinemaSeatData";
  ...
  return (
    <>
      <div className="flex flex-col gap-2 border-gray/50 rounded-lg p-5 relative items-center">
        {downStairsMiddleSectionData.rows.map((row, rowIndex) => (
          <div key={rowIndex} className="flex gap-1 md:gap-2">
            {row.seats.map((seat, seatIndex) => {
              const seatData = {
                ...
                price: seat.price,
                status: seat.status,
                position: downStairsMiddleSectionData.location,
                description: downStairsMiddleSectionData.description,
              };
              return (
                <Seat
                  key={seatIndex}
